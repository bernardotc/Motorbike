...
import static pbgLecture5lab_wrapperForJBox2D.BasicPolygon.mkPole;
import static pbgLecture5lab_wrapperForJBox2D.BasicPolygon.mkRegularRectangle;
...
public class BasicPhysicsEngineUsingBox2D {
	...
	public static MouseJoint mouseJointDef;
        ...
        public BasicParticle leftWheel;
        public BasicParticle rightWheel;
...
//			public BasicRectangle(double sx, double sy, double vx, double vy, double width, double height, double orientation, double angularVeloctiy, boolean improvedEuler, Color col, double mass) {
		...
		rectangle = new BasicPolygon(WORLD_WIDTH/2-5, .5f,0,0, r*.5f,Color.YELLOW, 1, rollingFriction, mkRegularRectangle(4, r*2), 4);
		...
		polygons.add(rectangle);
                ...
                //polygons.add(pole);
                leftWheel = new BasicParticle(WORLD_WIDTH / 2 -5, .2f, 0, 0, r*2, Color.red, 1, rollingFriction * 17);
                leftWheel.body.m_fixtureList.setFriction(60);
                rightWheel = new BasicParticle(WORLD_WIDTH / 2 +2 -5, .2f, 0, 0, r*2, Color.red, 1, rollingFriction * 17);
                rightWheel.body.m_fixtureList.setFriction(60);
		particles.add(leftWheel);
		particles.add(rightWheel);
                
                RevoluteJointDef leftJoint = new RevoluteJointDef();
                leftJoint.bodyA = rectangle.body;
                leftJoint.bodyB = leftWheel.body;
                leftJoint.collideConnected = false;
                leftJoint.enableMotor = true;
                leftJoint.localAnchorA.set(-.5f, 0);
                leftJoint.localAnchorB.set(0,0);
                world.createJoint(leftJoint);
                
                RevoluteJointDef rightJoint = new RevoluteJointDef();
                rightJoint.bodyA = rectangle.body;
                rightJoint.bodyB = rightWheel.body;
                rightJoint.collideConnected = false;
                rightJoint.enableMotor = true;
                rightJoint.localAnchorA.set(.5f, 0);
                rightJoint.localAnchorB.set(0,0);
		...
		switch (layout) {
			case RECTANGLE: {
				...
				barriers.add(new AnchoredBarrier_StraightLine(0, WORLD_HEIGHT, 0, 0, Color.WHITE, "left"));
                                ...
                                barriers.add(new AnchoredBarrier_StraightLine(5, 0, 6, 1, Color.GREEN));
                                barriers.add(new AnchoredBarrier_StraightLine(6, 1, 7, 1, Color.GREEN));
                                barriers.add(new AnchoredBarrier_StraightLine(7, 1, 8, 0, Color.GREEN));
				break;
			}
			case CONVEX_ARENA: {
				// These would be better created as a JBox2D "chain" type object for efficiency and potentially better collision detection at joints. 
	...
	public void update() {
		...
		int POSITION_ITERATIONS=NUM_EULER_UPDATES_PER_SCREEN_REFRESH;
                if (BasicKeyListener.isRotateLeftKeyPressed()) {
                        leftWheel.body.applyForceToCenter(new Vec2(-15, 0));
                        rightWheel.body.applyTorque(+12);
                        rectangle.body.applyTorque(-12);
                        rightWheel.body.applyForceToCenter(new Vec2(-15, 0));
                } else if (BasicKeyListener.isRotateRightKeyPressed()) {
                        leftWheel.body.applyForceToCenter(new Vec2(+15, 0));
                        leftWheel.body.applyTorque(-12);
                        rectangle.body.applyTorque(+12);
                        rightWheel.body.applyForceToCenter(new Vec2(+15, 0));
                }
